...
The integrated source passes all 2,332 repository tests, both typechecks and the frontend build. [PR gates and staging/browser smoke](https://github.com/grtwrn/yale-shuttle/actions/runs/34390917283) and [accuracy CI](https://github.com/grtwrn/yale-shuttle/actions/runs/34390917414) passed on `0768393`. Local test discovery excludes `store/**`, which contains private archived test sources; the clean CI checkout runs the unfiltered repository suite.
...
The [completed full-day Red comparison](red-rider-full-days-2026-09-09.md) preserves all four runs, original rider membership, paired original first issues and recording coverage. Winchester-to-Division first point-ETA MAE improves from 112.93 to 95.31 seconds on September 4 and from 105.92 to 63.72 seconds on September 8. Later ETA error worsens on September 4, and legacy point-format sequence diagnostics also regress; those are reported separately from the current range UI. The [later September 9 causal-history replay](red-phase-validation-later-2026-09-09.md) adds eight holds with a larger first-total and ETA gain, alongside increased overestimation and wider intervals. These results establish targeted first-estimate improvements, not a complete countdown-stability fix.

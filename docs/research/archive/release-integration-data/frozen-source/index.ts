...
export function beliefFor(
  ...
  let release: Parameters<typeof stepBelief>[6];
  const config = releaseExperiment(), pin = e.releasePin;
  if (ring.routeId === '3' && config.transition && pin && (!config.winchesterOnly || pin.stopId === 11)) {
    const age = lapAgesOf(bus as BusData)?.[pin.stopId];
    const d = releaseDist(pin.stopId, pin.since, age === undefined ? undefined : age - (now - pin.since) / 1000);
    if (d) release = { stopId: pin.stopId, dist: d, since: pin.since, shuffle: config.shuffle ?? false };
  }
  const b = stepBelief(e.belief, ring, bus, now, seq, release);
  e.belief = b;
  return b;
}
...
export function arrivalsForBus(
  ...
  const config = releaseExperiment();
  if (store && ring.routeId === '3' && config.smoothSec) {
    const entry = entryFor(store, key);
    const memory = entry.releaseSmoothing ??= new Map();
    const active = releasePin && belief.rested && ring.stops[belief.restStop] === releasePin.stopId && [11, 121].includes(releasePin.stopId);
    for (const row of rows) {
      ...
      if (active && old && dt >= 0 && dt <= 15 && row.stopsAhead <= old.row.stopsAhead) {
        const weight = 1 - Math.exp(-dt / config.smoothSec);

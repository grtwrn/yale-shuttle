...
import { type Dist, hazard } from "./dist";
...
export function stepBelief(
  ...
  release?: { stopId: number; dist: Dist; since: number; shuffle: boolean },
): Belief {
  ...
  if (fresh) {
    ...
    const inRestFix = (kerbShuffleEvidence || (release?.shuffle && release.stopId === restId)) && prev.rested && !leftRest;
    ...
    for (let c = 0; c < C; c++) {
      ...
      if (mStand > 1e-12) {
        ...
        if (canShuffle) {
          ...
          if (table) {
            const matched = release && stops[z.stop] === release.stopId && prev.restStop === z.stop;
            const hd = hazard(matched ? release.dist : table, matched ? Math.max(0, (prev.seenAt - release.since) / 1000) : stood) * dt;

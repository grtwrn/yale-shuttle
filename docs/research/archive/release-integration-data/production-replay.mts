...
for(const f of frames){
 ...
 for(const arm of arms){
  setSampledFutureLap(true);setReleaseModelEnabled(arm==='candidate');
...
fs.closeSync(output);const meta={arms,refitAt,unavailable,parityChecks,frames:n,emitted,changed,beliefChecks,gaps,params:params.version,priorCutoff:'2026-09-14T04:00:00Z',candidatePatch:'unchanged-marginals-plus-release-fit-for-Winchester',note:'Unrounded target-scoped estimator output with periodic full ServerEta parity checks; continuous within observed segments; no synthetic observations or watcher-run resets. Distributions/tables priorSep14; parameter version publishedbeforeSep16. Actual approximately five-second raw cadence. Includes both target occurrences; collector lacks prior-day seeds. Marginal tables unchanged. Winchester-only current-release model and30s absolutequantile pooling; lower-tail legacywidening retained. Coefficients refreshcausally at13:14ETSep17usingonlyalreadyavailableoutcomes. Otherroutebeliefs andfutureholds unchanged.'};

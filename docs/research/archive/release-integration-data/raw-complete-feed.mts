...
const root=process.cwd(),dir='/home/gwarren/projects/yale-shuttle-watcher/release-integration-data';
...
const net=loadNet();const rows=net.db.prepare("SELECT * FROM raw_positions WHERE route_id=3 AND collected_at>=? ORDER BY collected_at,id").all(Date.parse('2026-09-16T04:00:00Z'));
...
let collector=fresh(),prev=0,segment=0,events=0;const fd=fs.openSync(dir+'/raw-complete-frames.jsonl','w');const gapEvents:any[]=[];

...
try {
 ...
 await page.route('**/api/geocode**',r=>r.fulfill({json:{results:[{display_name:'LEPH / 60 College',lat:41.303735,lon:-72.932155,type:'university',class:'yale'}]}}));page.setDefaultTimeout(12000);const errors=[];page.on('pageerror',e=>errors.push(e.message));
 let samples=0;
 if(local && !process.argv.includes('--api')) await page.route('**/api/**', async route=> {
  if(route.request().url().includes('/api/buses')) {
   const p=JSON.parse(fs.readFileSync('/home/gwarren/projects/yale-shuttle-watcher/wait-route-data/cache-fix/production-live-02.json','utf8'));
  ...
  if(route.request().url().includes('/api/geocode')) return route.fulfill({json:{results:[{display_name:'LEPH / 60 College',lat:'41.2845',lon:'-72.9577',type:'hospital',class:'amenity'}]}});
  return route.fulfill({json:[]});
 });
 page.on('response',async r=>{if(r.url().endsWith('/api/buses')&&!local){try{const p=await r.json();assert.equal(r.headers()['cache-control'],'no-store');fs.writeFileSync(`/home/gwarren/projects/yale-shuttle-watcher/wait-route-data/cache-fix/live-payload-${++samples}.json`,JSON.stringify(p));}catch{}}});
 ...
 if(!local || process.argv.includes('--api')) for(let i=0;i<25;i++){const r=await inspect(); results.push(r);fs.writeFileSync(`/home/gwarren/projects/yale-shuttle-watcher/wait-route-data/cache-fix/production-${local?'local':'live'}-ticks.json`,JSON.stringify(results,null,2)); await page.waitForTimeout(700);}
 for(const width of [390,320,430]) {
  ...
  results.push({width,mode:'embedded',...await inspect()});await page.screenshot({path:`/home/gwarren/projects/yale-shuttle-watcher/wait-route-data/cache-fix/production-${local?'local':'live'}-${width}.png`});
 ...
 fs.writeFileSync(`/home/gwarren/projects/yale-shuttle-watcher/wait-route-data/cache-fix/production-${local?'local':'live'}-result.json`,JSON.stringify({results,errors,samples},null,2));
 const ticks=results.filter(r=>!r.width);assert(ticks.length>=20);const seconds=r=>{const m=r.text.match(/Waiting (\d+):(\d+)/);assert(m);return Number(m[1])*60+Number(m[2]);};
 const backsteps=ticks.slice(1).filter((r,i)=>r.title===ticks[i].title && seconds(r)<seconds(ticks[i]));
 ...
 for(const r of results.filter(r=>r.width)){assert.equal(r.collisions.length,0);assert(r.text.startsWith('Red · Waiting '));assert.equal(r.text.split('\n').length,2);}

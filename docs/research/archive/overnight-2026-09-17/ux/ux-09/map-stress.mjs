...
const distRoot = process.env.DIST_ROOT || service + '/web/dist';
...
let requests = 0;
const report = { scope:'Built SPA on bounded local server, normal browser clock; synthetic feed; no live backend', checks:[],states:[],errors:[], errorStacks:[], iterations:0 };
const server = http.createServer(async(req,res)=>{
 try {
  ...
  const file = path.resolve(distRoot, '.' + (url.pathname === '/' ? '/index.html' : url.pathname));
  if(!file.startsWith(distRoot + '/')) {res.writeHead(404);res.end();return;}
...
try {
 ...
 page=await context.newPage();page.setDefaultTimeout(10000);page.on('pageerror',e=>{report.errors.push(e.message);if(report.errorStacks.length<3)report.errorStacks.push(e.stack);});
 ...
 report.checks.push('Up to 3 normal-clock zoom-in/unmount and zoom-out/filter/unmount cycles completed; no page error or crash fallback');

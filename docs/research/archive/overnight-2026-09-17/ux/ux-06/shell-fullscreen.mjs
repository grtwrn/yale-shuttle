...
try{
 ...
 const savedDraft=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('shuttle-trip-draft')));
 await fsButton.focus();await page.keyboard.press('Enter');await page.locator('.map-fs').getByRole('button',{name:'Back',exact:true}).focus();await page.keyboard.press('Enter');await page.clock.runFor(150);assert(await fsButton.evaluate(e=>e===document.activeElement));
 await page.getByRole('button',{name:'View Red trip details',exact:true}).focus();await page.keyboard.press('Enter');
 const berthToggle=page.getByRole('button',{name:/Red stops about .*show where/});await berthToggle.waitFor();await berthToggle.focus();await page.keyboard.press('Space');
 const berth=page.locator('.berth-map-wrap');await berth.waitFor();await berth.getByRole('button',{name:'Full map',exact:true}).focus();await page.keyboard.press('Enter');await page.clock.runFor(150);await berth.getByRole('button',{name:'Back',exact:true}).focus();await page.keyboard.press('Enter');await page.clock.runFor(150);assert(await berth.getByRole('button',{name:'Full map',exact:true}).evaluate(e=>e===document.activeElement));
 const directions=page.getByRole('link',{name:'🧭 Directions to published stop',exact:true});assert.match(await directions.getAttribute('href'),/travelmode=walking/);assert.match(await directions.getAttribute('href'),new RegExp(String(feed.stop_coords[48].lat)));assert.match(await page.locator('body').innerText(),/#307/);
 await berth.getByRole('button',{name:'Full map',exact:true}).tap();await page.clock.runFor(150);await berth.getByRole('button',{name:'Back',exact:true}).tap();await page.clock.runFor(150);assert(await berth.getByRole('button',{name:'Full map',exact:true}).evaluate(e=>e===document.activeElement));
 await page.screenshot({path:out+'/shell-berth-390.png'});
 await page.getByTitle('Back to all routes',{exact:true}).click();await page.clock.runFor(150);await page.getByRole('button',{name:'View Red trip details',exact:true}).waitFor();
 const afterDraft=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('shuttle-trip-draft')));for(const key of ['fromText','fromLL','toText','toLL','tripTime'])assert.deepEqual(afterDraft[key],savedDraft[key],key+' retained');
 ...
 report.checks.push('Built-app overview and pickup-location Back restore toggle focus; directions retain published coordinate; All routes preserves endpoints and both shuttle identities');

...
const { parseOptions } = await import(service + '/scripts/canary-metrics.mjs');
...
let feedRequests=0, offline=false;
...
try{
 ...
 await page.route('**/*',async r=>{const u=new URL(r.request().url());if(u.hostname!=='shell-map.test')return r.abort();if(u.pathname==='/api/buses'){feedRequests++;if(offline)return r.abort();return r.fulfill({json:feed});}if(u.pathname==='/api/weather')return r.fulfill({status:204});if(u.pathname.startsWith('/api/'))return r.fulfill({json:{reports:[],results:[],routes:[]}});const f=u.pathname==='/'?'/index.html':u.pathname;try{return r.fulfill({body:await fs.readFile(service+'/web/dist'+f),contentType:f.endsWith('.js')?'text/javascript':f.endsWith('.css')?'text/css':'text/html'});}catch{return r.fulfill({status:404});}});
 ...
 await fs.writeFile(out+'/canary-body.txt',bodyText);
 report.canary=parseOptions(bodyText);
 assert.equal(report.canary.filter(o=>o.mode==='shuttle').length,1);
 assert.equal(report.canary[0].routeLabel,'Red');
 assert(report.canary[0].eta && report.canary[0].eta.first[1]>=report.canary[0].eta.first[0]);
 report.checks.push('Existing watcher parser reads the real card once, preserving Red and its window rather than counting map labels as cards');
 ...
 async function advance(ms=6000) { await page.clock.runFor(ms); await page.waitForTimeout(120); await page.clock.runFor(400); }
 async function fresh() {
  const t=await page.evaluate(()=>Date.now()); feed.server_eta.at=t;feed.server_eta.servedAt=t;
  feed.buses.forEach(b=>b.observed_at=t);
  await advance();
 }
 await fresh();
 assert.match(await page.locator('.trip-map-wrap').innerText(),/🚌 \(R\)/);
 assert.equal(await page.locator('.trip-map-wrap .bus-pin-sm').first().getAttribute('aria-label'),'Red #307');
 await page.getByRole('button',{name:/Red arrival details/}).first().click();
 await dialog.waitFor();assert.match(await dialog.innerText(),/Following shuttle · #309/);await page.keyboard.press('Escape');
 report.checks.push('Fresh recovery restores timing, selected bus #307 and following #309');
 offline=true; const requestsBefore=feedRequests;
 for(let i=0;i<10;i++) await advance(6000);
 assert(feedRequests>requestsBefore,'aborted feed requests attempted');
 const failedText=await page.locator('.trip-map-wrap').count()?await page.locator('.trip-map-wrap').innerText():'';
 assert(!failedText.includes('🚌 (R)'));
 assert.equal(await page.locator('.trip-map-wrap .bus-wait-label').count(),0);
 report.checks.push('Network failure beyond the 45-second freshness limit clears map timing');
 offline=false;await fresh();
 assert.match(await page.locator('.trip-map-wrap').innerText(),/🚌 \(R\)/);
 report.checks.push('Polling recovers map times after network failure');
 const savedBuses=feed.buses;feed.buses=[];feed.server_eta.buses=[];feed.server_eta.rows=[];feed.server_eta.distributions=[];
 await advance();
 assert.equal(await page.locator('.trip-map-wrap .bus-pin-sm').count(),0);
 assert.equal(await page.locator('.trip-map-wrap .bus-wait-label').count(),0);
 report.checks.push('Empty fleet removes prior bus markers and waiting identity');

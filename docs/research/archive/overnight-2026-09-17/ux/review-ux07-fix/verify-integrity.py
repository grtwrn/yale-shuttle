...
head='b8228f443e1d7a0a6a1e3fc3ade4346e20fe0b91'
...
for path in [root/files[1],out/'review-lifecycle.mjs',out.parent/'review-ux07/boundary-acceptance.mjs']:subprocess.run(['node','--check',str(path)],check=True)
prior=json.loads((out/'prior-evidence.json').read_text())
for p,h in prior.items():assert hashlib.sha256((out.parent/p).read_bytes()).hexdigest()==h,p
expected=json.loads((out.parent/'ux-07-fix/integrity.json').read_text())['sources']
assert sources==expected,'committed correction equals verified builder source'
assert git('diff','a401ef5c7fe675e59e0ec0e2b4dbb474ba176b34',head,'--',*files)==(out.parent/'ux-07-fix/proposal.patch').read_bytes()
images=[p for team in ['eta','ux'] for p in (out.parent.parent/team).rglob('*') if p.suffix.lower() in ['.png','.jpg','.jpeg','.webp']]
report={'head':head,'base':base,'parents':git('show','-s','--format=%P',head).decode().strip().split(),'sources':sources,**proof,'unchanged_numerical_modules':True,'poll_only_adds_presentation_flag':True,'all_routes_map_unchanged':True,'builder_artifacts_preserved':len(manifest),'all_prior_evidence_preserved':len(prior),'committed_correction_matches_builder_patch':True,'bundle_source_matches_head':True,'clean_checkout_index':True,'image_count':len(images),'image_bytes':sum(p.stat().st_size for p in images)}

...
const report={phase:prefix,states:[],checks:[],errors:[]};
try {
 ...
 await panel.waitFor();
 for(const [scenario,expected] of Object.entries({fresh:'Red may fit your buffer',buffer:'Your buffer may be tight',stale:'Live shuttle times unavailable',loading:'Live shuttle times unavailable',partial:'Some shuttle times are unavailable',failed:'Live shuttle times unavailable',missing:'Live shuttle times unavailable',future:'Live shuttle times unavailable',late:'You may arrive after class starts',catch:'Check the shuttle connection',estimated:'Allow extra time for this shuttle',walk:'Walk now',empty:'Arrival time unavailable','second-buffer':'Your buffer may be tight','second-catch':'Check the shuttle connection','both-cautions':'Check the shuttle connection','departed-unknown':'You may arrive after class starts','future-walk':'Walking fits your buffer'})){
  ...
  report.states.push({scenario,heading,text:await panel.innerText()});
  ...
  if(['second-buffer','second-catch','both-cautions'].includes(scenario))await page.screenshot({path:out+'/'+prefix+'-'+scenario+'-360.png'});
 }
 if(!baseline){
 ...
 report.checks.push('18 scenario states at 360/390/430/1280px without horizontal overflow');

...
try {
 ...
 const modeButton=page.getByRole('button',{name:'Running now',exact:true});
 await showSelected.focus();assert(await showSelected.evaluate(e=>e===document.activeElement));
 // A successful fleet update automatically reveals selected route cards.
 ...
 report.automaticRecovery={focus:await page.evaluate(()=>({tag:document.activeElement.tagName,text:document.activeElement.textContent?.slice(0,80)})),modeFocused:await modeButton.evaluate(e=>e===document.activeElement)};
 report.blockingFindings=[];
 if(!report.automaticRecovery.modeFocused)report.blockingFindings.push('Recovery button disappears after a fresh empty poll and loses keyboard focus to BODY instead of persistent map mode control');
 // Control: background recovery must not steal another focused button.
 feed.buses=buses;feed.server_eta=wire;await poll();await showSelected.waitFor();await nav('issues').focus();
 feed.buses=[];delete feed.server_eta;await poll();await page.locator('[id="route-card-Blue Day"]').waitFor();
 assert(await nav('issues').evaluate(e=>e===document.activeElement));
 report.externalFocusPreserved=true;
 // Red sends valid current positions, but every position is off the route.
 ...
 await poll();await showSelected.waitFor();
 report.offRoute={filteredText:await body(),chipTitle:await page.getByRole('button',{name:'Red',exact:true}).getAttribute('title')};
 await page.screenshot({path:out+'/off-route-filtered.png'});
 await showSelected.focus();await page.keyboard.press('Enter');await red.waitFor();
 report.offRoute.recoveredCard=await red.innerText();
 assert.match(report.offRoute.recoveredCard,/2.*bus/);assert.match(report.offRoute.recoveredCard,/off route/);
 if(report.offRoute.filteredText.includes('No buses are reporting on your selected routes.'))report.blockingFindings.push('Reporting off-route buses are described as no buses reporting in the Running now empty state');
 report.checks.push('Automatic recovery focus, external-focus control, and fresh reporting off-route fleet checked in built SPA');
 ...
 await fs.writeFile(out+'/boundary-acceptance.json',JSON.stringify(report,null,2));
}

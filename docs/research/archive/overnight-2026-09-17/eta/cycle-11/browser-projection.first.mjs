...
try{
 ...
 for(let i=0;i<report.states.length;i++){
  ...
  assert.deepEqual(existing,previous.states[i].option,'exact baseline option '+report.states[i].name);
 }
 ...
 assert.equal(await page.getByRole('button',{name:"I'm on #309",exact:true}).count(),1);
 const action=page.getByRole('button',{name:"I'm on #309",exact:true});await action.focus();await page.keyboard.press('Tab');

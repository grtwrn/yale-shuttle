...
let now=0;const realNow=Date.now;Date.now=()=>now;const seen=new Set(),report:any={batches:0,options:0,distinctSelectionBatches:0,scope:'Synthetic simultaneous access options on 100 saved frames; implementation isolation check, not observed rider alternatives.',examples:[]};
try{
 for(const r of records){
  ...
  const identities=new Set(combined.map((o:any)=>JSON.stringify(o.livePickupSelection)));
...
fs.writeFileSync(O+'multi-option.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({batches:report.batches,options:report.options,distinctSelectionBatches:report.distinctSelectionBatches}));

...
meta=json.loads((O/'current-resume6000-meta.json').read_text());top={int(k):v['stops'] for k,v in meta['routeMeta'].items()};labels={v['label']:int(k) for k,v in meta['routeMeta'].items()}
...
@lru_cache(None)
def latest_id(route,bus,at):
 return db.execute('select id from stop_visits where route_id=? and bus_name=? and anchored_at<=? order by anchored_at desc,id desc limit 1',(route,bus,at)).fetchone()['id']
checks=collections.Counter();examples=[];backward={};lastTrack={}
for arm in ['current','canonical']:
 boundary=json.loads((O/(arm+'-boundary5999.json')).read_text())
 ...
 for line in gzip.open(O/(arm+'-resume6000-wire.jsonl.gz'),'rt'):
  ...
  for r in wanted:
   ...
   if r.get('alignment'):
    proof=r['alignment'];track=f['tracking'][meta['routeMeta'][str(r['route'])]['label']+'|'+r['bus']]
    assert track[2] and track[3]==s['stop_index']==proof['originIndex'] and track[4]==s['pinned_at']
    assert (r['targetIndex']-wire[5])%N==s['stop_index'];expected=wire[5]
    cur=s
    for lid in proof['proofLegIds']:cur=next_visit(cur['id'],lid)
    assert cur['id']==proof['historicalSourceId']==r['historicalSourceId']
    assert sum(leg(i)['hops'] for i in proof['proofLegIds'])==proof['sourceOffsetHops']
    assert latest_id(r['route'],s['bus_name'],r['at'])==cur['id'];checks[arm+':exactLaggedOrigin']+=1
   elif same:

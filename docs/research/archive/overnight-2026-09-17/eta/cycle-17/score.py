...
meta=json.loads((O/'current-resume6000-meta.json').read_text());top={int(k):v['stops'] for k,v in meta['routeMeta'].items()}
...
hopById={l['id']:l['hops'] for ll in legs.values() for l in ll};hopCache={}
...
def chain_hops(source,ls):
 if source['id'] not in hopCache:
  cs=[0]
  for lid in ls:cs.append(cs[-1]+hopById[lid])
  hopCache[source['id']]=cs
 return hopCache[source['id']]
...
pinVisits=collections.defaultdict(list)
for v in visits.values():
 if v['pinned_at'] is not None:pinVisits[(v['route_id'],v['bus_name'].lstrip('#'),v['stop_index'],v['pinned_at'])].append(v)
alignmentCache={}
def lagged_origin(route,bus,source,t,origin,at):
 # A clock alone is not an identity: require the priced physical origin, the
 # active rest, the unique exact pin, and a connected path into latest source.
 cacheKey=(source['id'],t[2],t[3],t[4],origin)
 if cacheKey in alignmentCache:return alignmentCache[cacheKey]
 result=None
 if t[2] and t[3]==origin:
  pp=[v for v in pinVisits.get((route,bus,origin,t[4]),[]) if v['how']!='gap' and v['anchored_at']<source['anchored_at'] and v['departed_at'] is not None and v['departed_at']<=source['anchored_at']]
  if len(pp)==1:
   prior=pp[0];vv,ll,_=chain(prior);ids=[v['id'] for v in vv]
   if source['id'] in ids:
    si=ids.index(source['id']);cs=chain_hops(prior,ll)
    result={'source':prior,'visits':vv,'legs':ll,'sourceOffsetHops':cs[si],'proofLegIds':ll[:si],'pricedOrigin':origin}
 alignmentCache[cacheKey]=result
 return result

def q(xs,p):
 xs=sorted(xs);i=(len(xs)-1)*p;lo=math.floor(i);return xs[lo]+(xs[math.ceil(i)]-xs[lo])*(i-lo)
metrics=collections.defaultdict(lambda: {'current':[],'canonical':[],'source':set(),'target':set()});counts=collections.Counter();availability=collections.Counter();flips=collections.Counter();stability=collections.Counter();previous={};unpairedFile=(O/'unpaired-arrivals.jsonl').open('w');trackingFile=(O/'tracking-changes.jsonl').open('w');tailFile=(O/'large-tail-changes.jsonl').open('w');scoredFile=(O/'connected-pairs.jsonl').open('w');worst=[];progress=collections.Counter();lastTrack={};sampleCases=[];identityFile=(O/'unresolved-source-identities.jsonl').open('w');alignmentFile=(O/'lagged-origin-records.jsonl').open('w');transitionFile=(O/'post-arrival-transitions.jsonl').open('w');alignmentCounts=collections.Counter()
# Only seed stability/tracking from the verified prior terminal poll. No old poll is rescored.
for arm in ['current','canonical']:
 boundary=json.loads((O/(arm+'-boundary5999.json')).read_text());kk,_=keyed(boundary)
 for key,(r,t) in kk.items():previous[(arm,key)]=(boundary['at'],boundary['at']/1000+r[2],r[5])
 for key,t in boundary['tracking'].items():lastTrack[(arm,key)]=(boundary['at'],t[1])
for aa,bb in zip(gzip.open(O/'current-resume6000-wire.jsonl.gz','rt'),gzip.open(O/'canonical-resume6000-wire.jsonl.gz','rt'),strict=True):
 ...
 for key in ka.keys()|kb.keys():
  ...
  band=max(abs(ra[j]-rb[j]) for j in [2,3,4]);truth=None;source=None;historicalSource=None;target=None;legIds=[];reason='unsampled';alignment=None
  if a['sample'] or band>60:
   times0=times.get((route,bus),[]);si=bisect.bisect_right(times0,at)-1
   if si<0:reason='no-source'
   else:
    source=bybus[(route,bus)][si];historicalSource=source;vs,ls,reason=chain(source)
    N=len(top[route]);proofs=[lagged_origin(route,bus,source,t,(idx-r[5])%N,at) for r,t in [(ra,ta),(rb,tb)]]
    matched=[]
    if any(proofs):
     # Correct ALL target rows of an established lagged rest, not just zero-hop
     # rows. Absolute forecast hops identify the visit on this prior chain.
     if all(proofs) and proofs[0]['source']['id']==proofs[1]['source']['id']:
      proof=proofs[0];source=proof['source'];vs=proof['visits'];ls=proof['legs'];cs=chain_hops(source,ls)
      alignment={'kind':'exact-lagged-rest','historicalSourceId':historicalSource['id'],'pricedSourceId':source['id'],'originIndex':proof['pricedOrigin'],'sourceOffsetHops':proof['sourceOffsetHops'],'proofLegIds':proof['proofLegIds'],'restSince':[ta[4],tb[4]]}
      for r in [ra,rb]:
       ti=cs.index(r[5]) if r[5] in cs else None
       if ti is not None:assert vs[ti]['stop_index']==idx
       matched.append((ti,vs[ti]) if ti is not None else None)
     else:
      reason='arm-origin-alignment-unresolved';matched=[None,None]
    else:
     # Legacy alignment retained outside the positively established correction.
     # It remains provisional when no recorded pin establishes the priced visit.
     for arm,kk in [('current',ka),('canonical',kb)]:
      ...
      if sameSource and firstHop is not None and 0<firstHop<N:
       matched.append(None);continue
      keepSource=sameSource and firstHop==0
      firstDistance=(idx-source['stop_index'])%N
      if sameSource and not keepSource:firstDistance=N
      expectedDistance=firstDistance+occ*N;cs=chain_hops(source,ls)
      ti=cs.index(expectedDistance) if expectedDistance in cs else None
      if ti is not None:assert vs[ti]['stop_index']==idx
      matched.append((ti,vs[ti]) if ti is not None else None)
    ...
    elif source['stop_index']==idx or any(proofs):
     reason='lagged-target-identity-unresolved' if alignment else 'arm-origin-alignment-unresolved' if any(proofs) else 'source-target-identity-unresolved'
     identityFile.write(json.dumps({'at':at,'key':key,'sourceId':source['id'],'historicalSourceId':historicalSource['id'],'alignment':alignment,'reason':reason,'firstHops':[ka.get((route,bus,idx,0),(None,None))[0][5] if (route,bus,idx,0) in ka else None,kb.get((route,bus,idx,0),(None,None))[0][5] if (route,bus,idx,0) in kb else None],'possibleTargetIds':[x[1]['id'] if x else None for x in matched],'chainVisitIds':[v['id'] for v in vs if v['stop_index']==idx]})+'\n')
  ...
  if alignment:
   record['alignment']=alignment;record['historicalSourceId']=historicalSource['id'];alignmentFile.write(json.dumps(record)+'\n');alignmentCounts['rows']+=1;alignmentCounts['minuteRows']+=a['sample'];alignmentCounts['zeroHopRows']+=ra[5]==0;alignmentCounts['nonzeroRows']+=ra[5]>0
  ...
  if truth<0:counts['postArrivalTransition']+=1;transitionFile.write(json.dumps(record)+'\n');continue
...
alignmentFile.close();transitionFile.close();identityFile.close();tailFile.close();scoredFile.close();unpairedFile.close();trackingFile.close();summary={}

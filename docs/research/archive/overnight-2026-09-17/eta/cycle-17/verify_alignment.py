...
@lru_cache(None)
def pin_ids(route,bus,idx,pin):
 return [x['id'] for x in db.execute('select id from stop_visits where route_id=? and bus_name=? and stop_index=? and pinned_at=?',(route,bus,idx,pin))]
@lru_cache(None)
...
for arm in ['current','canonical']:
 ...
 for line in gzip.open(O/(arm+'-resume6000-wire.jsonl.gz'),'rt'):
  ...
  for r in list(pending[1]):
   ...
   assert pin_ids(route,s['bus_name'],s['stop_index'],s['pinned_at'])==[s['id']]
   assert latest_id(route,s['bus_name'],r['at'])==later['id']
...
result={'counts':counts,'alignedBusPolls':len(proofPairs),'priorVisits':len(priorIds),'limitations':'New6000:9000 alignment checks only. Previous19reviewer cases are preserved and not regenerated. Other source identities remain provisional.'}

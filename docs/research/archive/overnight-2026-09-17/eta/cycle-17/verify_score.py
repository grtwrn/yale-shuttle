...
meta=json.loads((O/'current-resume6000-meta.json').read_text());top={int(k):v['stops'] for k,v in meta['routeMeta'].items()};expected={r['id']:r['repairedStops'] for r in json.loads((O.parent/'cycle-14/all-route-topology.json').read_text())}
...
def successors(route,bus,idx,anchored,arrived):
 return list(db.execute("select * from stop_visits where route_id=? and bus_name=? and stop_index=? and anchored_at>=? and anchored_at<=? and (how is null or how!='gap') and (arrived_at=? or (arrived_at is null and outcome='passed' and departed_at=?))",(route,bus,idx,anchored,arrived,arrived,arrived)))
checked=set();sources=set();targets=set();rows=0;legUses=0;totals=collections.Counter();tails=0
for line in (O/'connected-pairs.jsonl').open():
...
score=json.loads((O/'score.json').read_text());assert rows==score['counts']['scored']
for arm in ['current','canonical']:

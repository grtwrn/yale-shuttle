...
O=Path(__file__).resolve().parent;B=O.parents[1]/'cycle-4'
...
followingOrder={}
for arm,d in [('baseline',ar),('candidate',br)]:
 counts=collections.Counter()
 for at,m in d.items():
  grouped=collections.defaultdict(list)
  for (bus,stop,hops),v in m.items():grouped[bus,stop].append((hops,v['eta']))
  for rs in grouped.values():
   if len(rs)==2:
    counts['bothRows']+=1;rs.sort();counts['secondBeforeFirst']+=rs[1][1]<rs[0][1]
 followingOrder[arm]=dict(counts)
prior=load(O.parents[1]/'cycle-2/trace-scores.json');checks=[];outside=[]
for r in prior['checkpoints']:
 if r['at'] not in ar:outside.append(r);continue
 key=(next(c['bus'] for c in load(O.parents[1]/'cycle-2/trace-outcomes.json')['cases'] if c['sourceId']==r['sourceId']).lstrip('#'),r['target'],r['current']['stopsAhead'])
...
jumps={}
for c in load(O.parents[1]/'cycle-2/trace-outcomes.json')['cases']:
...
summary=dict(frames=len(a),rows=same+len(changed),unchangedRows=same,changedRows=len(changed),zeros=dict(zeros),selectedCheckpoints=len(checks),outsideFrozenWindows=len(outside),priorMissingForecasts=len(prior['missingForecasts']),unconnectedEndpoints=len(prior['unconnectedEndpoints']),groups=groups,jumpSummary=jumpSummary,followingOrder=followingOrder,scope='Selected reused historical Red windows, same continuous-prefix calibration and raw inputs, rounded production wire. No population, new holdout or calibrated coverage claim. Checkpoint selection reused cycle2; no outcome exclusion.')

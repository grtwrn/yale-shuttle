...
const {setReleaseModelEnabled}=await import(pathToFileURL(root+'/web/src/eta/release.ts').href);
const earlyFits=JSON.parse(fs.readFileSync('/home/gwarren/projects/yale-shuttle-watcher/release-integration-data/runtime-fits-pre-sep10.json','utf8')).fits;
const freshFits=JSON.parse(fs.readFileSync('/home/gwarren/projects/yale-shuttle-watcher/release-integration-data/runtime-fits-development.json','utf8')).fits;
const refitAt=1789665240913;
const outDir='/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/cycle-5/traversal-guard';
const {computeUpcomingArrivals:baselineArrivals}=await import('/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/cycle-5/traversal-guard/baseline-arrivals.generated.mts');
const {ServerEta:BaselineServerEta}=await import('/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/cycle-5/traversal-guard/baseline-server.generated.mts');
...
class FocusEngine {
 ...
 contribute(payload:any,version:number,at:number){
  ...
  const arrivals=(this.arm==='baseline'?baselineArrivals:computeUpcomingArrivals)([48,4],buses,payload.routes,payload.stop_coords,payload.segments,at,payload.dwells,this.store);
  for(const[k,t]of this.seen)if(at-t>600000){this.seen.delete(k);this.store.delete(k);}
  if(version%1000===0){
   const server:any=new (this.arm==='baseline'?BaselineServerEta:ServerEta)({routes:['Red']});for(const[k,e]of this.store)server.store.set(k,structuredClone(e));
...
let engines:any={},firstAt=0,last=0,segmentId=0;const perBus=new Map();const output=fs.openSync(outDir+'/'+(process.env.REPLAY_NAME??'full')+'-pairs.jsonl','w');
...
for(const f of frames){
 ...
 if(!last||at-last>60000){engines=Object.fromEntries(arms.map(a=>[a,new FocusEngine(a)]));firstAt=at;segmentId++;perBus.clear();gaps.push({at,gapMs:last?at-last:null});}
 ...
 for(const arm of arms){
  ...
  const fit=(at>refitAt?freshFits:earlyFits).find((f:any)=>f.stopId===11);
  candidate.dwells['3']['11'].release=fit;
...
fs.closeSync(output);const meta={arms,refitAt,unavailable,parityChecks,frames:n,emitted,changed,beliefChecks,gaps,params:params.version,priorCutoff:'2026-09-14T04:00:00Z',candidatePatch:'unchanged-marginals-plus-release-fit-for-Winchester',note:'BOTH arms use current production release ON. Baseline copied mechanically from current HEAD with import paths rewritten; candidate native checkout. Historical afternoon already used. Unrounded target-scoped estimator output with periodic full ServerEta parity checks; continuous within observed segments; no synthetic observations or watcher-run resets. Distributions/tables priorSep14; parameter version publishedbeforeSep16. Actual approximately five-second raw cadence. Includes both target occurrences; collector lacks prior-day seeds. Marginal tables unchanged. Winchester-only current-release model and30s absolutequantile pooling; lower-tail legacywidening retained. Coefficients refreshcausally at13:14ETSep17usingonlyalreadyavailableoutcomes. Otherroutebeliefs andfutureholds unchanged.'};
fs.writeFileSync(outDir+'/'+(process.env.REPLAY_NAME??'full')+'-meta.json',JSON.stringify(meta,null,2));console.log(JSON.stringify(meta));

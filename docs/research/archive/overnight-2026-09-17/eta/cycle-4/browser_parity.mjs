 ...
 for(const sid of ['58224:48:0','58224:48:150']){
  ...
  try{
   ...
   for(let i=0;i<timeline.length;i++){
    ...
    }catch(e){await fs.writeFile(out+'/browser-failure.json',JSON.stringify({sid,i,at:active.at,want,arrays:await page.evaluate(()=>window.__parityArrays),text:await page.locator('body').innerText()},null,2));throw e;}
...
}finally{await browser.close();await fs.writeFile(out+'/browser-parity.json',JSON.stringify(report,null,2)+'\n');}

 ...
 for(const line of fs.readFileSync(archive+'/conditional-replay-data/raw-frames.jsonl','utf8').trim().split('\n')) {
  ...
  if(contexts.length){
   for(const b of buses){
    const old=original.get(at+'|'+b.bus_name.replace(/^#/,''));if(!old)continue;
    const bi=wire?.buses.findIndex((b:any)=>b[0]===old.bus.replace(/^#/,''));
    for(const a of old.forecasts){
     const ri=wire?.rows.findIndex((r:any)=>r[0]===bi&&r[1]===a.target&&r[5]===a.stopsAhead),r=wire?.rows[ri];
     if(!r){omissions.push({at,bus:b.bus_name,target:a.target,stopsAhead:a.stopsAhead});continue;}
     if(r[2]!==Math.round(a.eta)||Math.max(0,r[3])!==Math.round(a.low)||r[4]!==Math.round(a.high)||r[7]!==Math.round(a.departNow))throw Error('Round2 parity mismatch '+JSON.stringify({at,b:b.bus_name,a,r}));
     parity++;
     if(a.distribution){if(JSON.stringify(wire.distributions[ri])!==JSON.stringify(a.distribution.map(Math.round)))throw Error('Distribution mismatch');quantileParity++;}
    }
   }
...
const meta={head:plan.head,frames:n,emitted,rows,parity,quantileParity,names:[...names],gaps,endAt,elapsedSec:(performance.now()-started)/1000,provenance:'Actual recorded GPS; reconstructed collector clocks. Current code/historical causal calibration. No candidate.'};

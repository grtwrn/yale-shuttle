/** Reconstruct recorded five-second all-route polls using production collector reducers.
 * Read-only outcome DB; no future history seeds, no network and no persistence.
 */
import fs from'node:fs';import{pathToFileURL}from'node:url';
const root=process.cwd(),dir='/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/cycle-14';
...
const net=loadNet();const rows=net.db.prepare("SELECT * FROM raw_positions WHERE collected_at>=? ORDER BY collected_at,id").all(Date.parse('2026-09-16T04:00:00Z'));
...
let collector=fresh(),prev=0,segment=0,events=0;const fd=fs.openSync(dir+'/all-route-raw-frames.jsonl','w');const gapEvents:any[]=[];
for(const g of groups){
 ...
 const buses=[...collector.livePositions.values()].map((b:any)=>{
  ...
  for(const [stop,age] of Object.entries(ages??{}))lap[stop]=age;

...
report=dict(currentBase=plan['base'],identicalInputWindows=4,framesPerWindow=78,rowsPerWindow=sum(len(f['wire']['rows']) for f in first) if 'wire' in first[0] else sum(len(f['expected']['rows']) for f in first),checks=checks,canonicalExactFullStateAndWire=True,allBandsAndQuantilesOrdered=True,allBeliefsNormalized=True,cleanCheckout=True)

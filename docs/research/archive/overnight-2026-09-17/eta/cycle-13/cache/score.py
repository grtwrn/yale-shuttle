...
def compare(a,b):
 ...
 changedFrames=wireBuses=beliefs=0;maxP=0;rows=0;changedRows=0;availability=[];worst=[];occstats={};quantileDiffs=0;maxQ=0;columnMax={i:0 for i in [2,3,4,7,8]}
 for x,y in zip(aa,bb):
  ...
  for k in kr.keys()&kb.keys():
   ...
   for i in columnMax:
    if r[i] is not None and s[i] is not None:columnMax[i]=max(columnMax[i],abs(r[i]-s[i]))
   ...
   q1=x['wire']['distributions'][x['wire']['rows'].index(r)];q2=y['wire']['distributions'][y['wire']['rows'].index(s)]
   assert len(q1)==len(q2)==50
   quantileDiffs+=q1!=q2;maxQ=max(maxQ,max(abs(u-v) for u,v in zip(q1,q2)))

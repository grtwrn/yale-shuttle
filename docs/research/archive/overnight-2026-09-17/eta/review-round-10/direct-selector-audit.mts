...
const out=root+'/review-round-10';
...
const census=lines(out+'/paired-metadata.jsonl');
 ...
 for(let i=0;i<inputs.length;i++) {
  ...
  for(const arm of ['historical','missingDestination']) {
   ...
   if(here&&o.walkToSec<=dwellBoardWindowSec(here,cfg.routeIds[0],o.boardStopId,payload.dwells)) {
    assert.equal(got.livePickupSelection.relation,'raw-current');assert.equal(got.livePickupSelection.boarding.source,'raw-at-stop');
    assert.equal(got.livePickupSelection.boarding.busName,norm(here.bus_name));assert.equal(got.waitSec,0);counts.raw++;
   } else {
    ...
    for(const [name,row] of [['countdown',p.match],['boarding',p.boardable]] as const) {
     const evidence=got.livePickupSelection[name];assert.equal(evidence.source,'forecast');
   ...
   assert.equal(got.livePickupSelection.selectedAtMs,current);counts.checked++;
  }
 }

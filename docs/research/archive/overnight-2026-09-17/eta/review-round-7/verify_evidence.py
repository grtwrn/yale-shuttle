...
base='2dbc060bf517225c219fba8cbc5cb343fb19312b';head='1b66a1914a92a79c0936cf65473617cf7b7695f9';smooth='b574a1c800654544685a8d9f1097ad3484ac13fc';join='fd00a7c999f5622362579a46fca26be0c6e0f036'
...
assert git('diff',base,head,'--',*smoothing)==git('diff',smooth+'^',smooth,'--',*smoothing)
for f in ['docs/server-side-eta.md','services/shuttle-v2/web/src/journeyArrival.ts','services/shuttle-v2/web/src/atStopJourney.test.ts']:
 assert git('show',head+':'+f)==git('show',join+':'+f)
f='services/shuttle-v2/web/src/TransitMap.tsx'
src=(root/f).read_text(); before=git('show',base+':'+f).decode()
expected=before.replace('import { journeyArrival }','import { atStopJourneyBoard, journeyArrival }')
expected=expected.replace('        const arrival = journeyArrival(board, visits, o.alightStopId, effectiveWalkToSec, o.walkFromSec, nowMs);','        const forecastBoard = atStopJourneyBoard(visits, o.routeLabel, hereBus.bus_name, o.boardStopId, o.alightStopId);\n        const arrival = journeyArrival(forecastBoard, visits, o.alightStopId, effectiveWalkToSec, o.walkFromSec, nowMs);')
assert src==expected,'unexpected merged shell edit'
a=src.index('    return stableOptions.map((o) => {',src.index('const options: TripOption[] | null'));b=src.index('\n    // eslint-disable-next-line',a)
block=src[a:b]
assert hashlib.sha256(block.encode()).hexdigest()==read(C/'source-extraction.json')['block_sha256']
...
for p,h in read(O/'initial-hashes.json').items():assert hashlib.sha256((O.parent/p).read_bytes()).hexdigest()==h,p
db.close()
result=dict(head=head,base=base,priorSmoothingDiffExact=True,mergedShellDiffExact=True,prototypeDecisionsExact=len(current),missingCases=len(cases),connectedLegUses=connected_uses,connectedOutcomes=len(recomputed),changedOutcomes=len(changed),changedMAE=[statistics.mean(abs(r[k]) for r in changed) for k in ['e0','e1']],largestRetainedRegression=reg,builderArtifactsPreserved=True,scope='Selected reused Red history; no population coverage, fresh holdout, or walking-outcome claim.')

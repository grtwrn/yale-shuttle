  ...
  for c in selected.get((arm,f['i']),[]):
   ...
   if len(pins)==1:
    ...
    if p['stop_index']==origin:
     ...
     directProofs.append({'arm':arm,'key':key(r),'oldTargetId':r['targetId'],'priorVisit':p,'latestSource':s,'directLeg':matching[0] if matching else None,'wireOrigin':origin,'trackingRestIndex':track[3],'trackingRestSince':track[4],'priorArrivalRelativeSec':(p['arrived_at']-r['at'])/1000})

...
O=Path(__file__).resolve().parent;P=Path('/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta')
...
for arm in ['current','canonical']:
 ...
 for a,b in zip(warm,cold):
  wa,wb=a['wire'],b['wire']
  ka={(wa['buses'][r[0]][0],r[1],r[5]):(r,q) for r,q in zip(wa['rows'],wa['distributions'])}
  kb={(wb['buses'][r[0]][0],r[1],r[5]):(r,q) for r,q in zip(wb['rows'],wb['distributions'])}
  assert set(ka)==set(kb) and len(ka)==len(wa['rows'])
  for key,(ra,qa) in ka.items():
   rb,qb=kb[key]
   changed+=ra!=rb

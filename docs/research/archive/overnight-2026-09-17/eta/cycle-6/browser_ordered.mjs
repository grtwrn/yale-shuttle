...
try{
 for(const sid of ['57454:48:0','58224:48:0']){
  ...
  try{
   ...
   for(let i=0;i<timeline.length;i++){
    ...
    }catch(e){await fs.writeFile(reportOut+'/ordered-browser-failure.json',JSON.stringify({sid,i,at:active.at,want,arrays:await page.evaluate(()=>window.__parityArrays),text:await page.locator('body').innerText()},null,2));throw e;}
...
}finally{await browser.close();await fs.writeFile(reportOut+'/ordered-browser-parity.json',JSON.stringify(report,null,2)+'\n');}

...
const reportOut='/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/review-round-6';
...
const decisions=(await fs.readFile(reportOut+'/ordered-browser-decisions.jsonl','utf8')).trim().split('\n').map(JSON.parse);
 ...
 for(const sid of ['57454:48:0']){
  ...
  try{
   ...
   await page.route('**/*',async route=>{
    ...
    try{return route.fulfill({body:await fs.readFile(reportOut+'/ordered-dist'+file),contentType:file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':'text/html'});}catch{return route.fulfill({status:404});}
   });
   ...
   for(let i=0;i<timeline.length;i++){
    ...
    }catch(e){await fs.writeFile(reportOut+'/ordered-browser-transitions-failure.json',JSON.stringify({sid,i,at:active.at,want,arrays:await page.evaluate(()=>window.__parityArrays),text:await page.locator('body').innerText()},null,2));throw e;}
    ...
    if(i===timeline.length-1){
      ...
      await page.getByText('39 min',{exact:true}).waitFor();assert.equal(await page.getByText('ETA unavailable',{exact:true}).count(),0);
...
}finally{await browser.close();await fs.writeFile(reportOut+'/ordered-browser-transitions.json',JSON.stringify(report,null,2)+'\n');}

 ...
 for(const sid of ['57454:48:0','58224:48:0']){
  ...
  try{
   ...
   for(let i=0;i<timeline.length;i++){
    ...
    }catch(e){await fs.writeFile(reportOut+'/browser-failure.json',JSON.stringify({sid,i,at:active.at,want,arrays:await page.evaluate(()=>window.__parityArrays),text:await page.locator('body').innerText()},null,2));throw e;}
    session.frames++;report.checks++;
    if(!want.options.find(o=>o.mode==='shuttle').journeyArrival)session.missingStates.push({at:active.at,text:await page.locator('body').innerText(),option:project(want.options).find(o=>o.mode==='shuttle')});
...
}finally{await browser.close();await fs.writeFile(reportOut+'/browser-parity.json',JSON.stringify(report,null,2)+'\n');}

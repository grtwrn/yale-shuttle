...
head='0d0f34fa87f7d051d4c4419f04c83db13c0d4c72'
base='373505d5076f21a08de69d9587be9b2a55956642'
assert git('rev-parse','HEAD').decode().strip()==head
assert git('rev-parse','MERGE_HEAD').decode().strip()==base
assert git('diff')==b'', 'worktree must equal resolved index'
assert git('ls-files','--unmerged')==b'', 'all conflicts must be resolved'
expected_ux={'services/shuttle-v2/scripts/trip-identity-check.mjs','services/shuttle-v2/web/src/TransitMap.tsx','services/shuttle-v2/web/src/TripBoardingActions.tsx','services/shuttle-v2/web/src/tripBusIdentity.test.ts','services/shuttle-v2/web/src/tripBusIdentity.ts'}
...
assert set(git('diff','--cached','--name-only',head).decode().splitlines())==expected_ux
assert set(git('diff','--name-only',base).decode().splitlines())==expected_eta
f='services/shuttle-v2/web/src/TransitMap.tsx';src=(root/f).read_text()
ours=git('show',head+':'+f).decode();theirs=git('show',base+':'+f).decode()
def numerical(text):
 a=text.index('    return stableOptions.map((o) => {',text.index('const options: TripOption[] | null'))
 b=text.index('\n    // eslint-disable-next-line',a)
 return text[a:b]
block=numerical(src)
assert block==numerical(ours), 'ETA numerical block must equal captured correction'
...
assert src==expected, 'entire integrated shell must contain exactly both approved deltas'
for p in expected_ux-{f}:assert (root/p).read_bytes()==git('show',base+':'+p),p
for p in expected_eta-{f}:assert (root/p).read_bytes()==git('show',head+':'+p),p
...
source_hashes={p:hashlib.sha256((root/p).read_bytes()).hexdigest() for p in sorted(expected_eta|expected_ux)}

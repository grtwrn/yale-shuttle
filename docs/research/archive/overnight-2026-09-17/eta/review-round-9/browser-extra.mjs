...
const desktop=process.env.REVIEW_DESKTOP==='1';
...
try{
 context=await browser.newContext({viewport:desktop?{width:1280,height:900}:{width:390,height:844},isMobile:!desktop,hasTouch:!desktop,timezoneId:'America/New_York',serviceWorkers:'block'});await seedTestId(context);
 ...
 // Reviewer assertions target the visible wait chip, rather than trusting the
 // option object or inspecting a whole-body text dump after the run.
 for (const name of ['different selected bus with destination','destination and distinct bus recover','fresh forecast recovers']) {
  const state=report.states.find(s=>s.name===name);assert.match(state.text,/⏳ 17 min(?:\n|$)/);assert.match(state.text,/🚌 #309 ·/);
 }
 for (const name of ['destination disappears, wait still belongs to #309','same vehicle later visit, first visit remains countdown','same vehicle later visit without destination']) {
  const state=report.states.find(s=>s.name===name);assert.match(state.text,/⏳ now-2 min(?:\n|$)/);assert.match(state.text,/🚌 #307 ·/);
 }
 assert.doesNotMatch(report.states.find(s=>s.name==='forecast expires, live identity unavailable').text,/⏳ /);
 report.viewport=desktop?'desktop':'mobile';report.renderedWaitAssertions=7;
 // Observe focus behavior when the destination disappears with the distinct
 // boarding action focused. Existing manual escape hatch must survive.
 await page.getByRole('button',{name:"I'm on #309",exact:false}).focus();
 feed.server_eta.rows=allRows.filter(r=>r[1]!==target);refreshDots();await refresh();
 await getOption({journey:false,close:true});
 await page.waitForFunction(()=>document.activeElement?.textContent?.includes("I'm on it"));
 report.manualFocusReturned=true;
 assert.equal(await page.getByRole('button',{name:"I'm on it",exact:false}).count(),1);
 ...
 await fs.writeFile(O+(desktop?'/browser-extra-desktop.json':'/browser-extra-mobile.json'),JSON.stringify(report,null,2)+'\n');
}

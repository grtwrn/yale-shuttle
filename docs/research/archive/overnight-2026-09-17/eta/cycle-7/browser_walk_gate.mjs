...
const O='/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/cycle-7';
const C=O.replace('cycle-7','cycle-6'),B=O.replace('cycle-7','cycle-5/traversal-guard'),service=process.cwd();
...
try{
 for(const walkSec of [0,100,119,121,150]){
  ...
  try{
   ...
   if(walkSec<=119){
    ...
    assert.equal(option.journeyArrival.catchRisk,walkSec>0);
   }else{
   ...
   if(walkSec===100 || walkSec===119){
    ...
    await section.getByRole('button',{name:/Red · #309/}).getByText(/Connection uncertain/).waitFor();
...
}finally{await browser.close();report.resourcesClosed=true;await fs.writeFile(O+'/walk-gate-browser.json',JSON.stringify(report,null,2)+'\n');}

  ...
  if a['sample'] or band>60:
   ...
   if si<0:reason='no-source'
    ...
    for arm,kk in [('current',ka),('canonical',kb)]:
     ...
     firstDistance=(idx-source['stop_index'])%len(top[route])
     if sameSource and not keepSource:firstDistance=len(top[route])
     expectedDistance=firstDistance+occ*len(top[route])
     cs=chain_hops(source,ls)

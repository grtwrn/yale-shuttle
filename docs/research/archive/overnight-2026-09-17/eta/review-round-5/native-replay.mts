...
for(const l of fs.readFileSync(A+'/conditional-replay-data/raw-frames.jsonl','utf8').trim().split('\n')){
 ...
 if(expected.has(at)){const e=expected.get(at)!;assert.deepEqual(f.buses,e.buses);assert.equal(JSON.stringify(actual),JSON.stringify(e.server_eta));matched++;rows+=actual.rows.length;}

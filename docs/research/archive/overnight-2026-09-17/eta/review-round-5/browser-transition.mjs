...
try{
 for(const sid of ['58224:48:0']){
  const expected=decisions.filter(d=>d.session===sid && d.at<=1789567342286), atSet=new Set(expected.map(d=>d.at));
  ...
  try{
   ...
   const session={id:sid,frames:0,firstText:null,lastText:null,changedState:null};report.sessions.push(session);
   for(let i=0;i<timeline.length;i++){
    ...
    }catch(e){await fs.writeFile('/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/review-round-5'+'/browser-transition-failure.json',JSON.stringify({sid,i,at:active.at,want,arrays:await page.evaluate(()=>window.__parityArrays),text:await page.locator('body').innerText()},null,2));throw e;}
    ...
    if(i===timeline.length-1){
      ...
      assert.equal(active.at,1789567342286);
      ...
      const text=await dialog.innerText();assert.ok(text.includes('At your stop'));assert.ok(text.includes('#309'));assert.ok(text.includes('Following shuttle')||text.includes('Next pass by this shuttle'));
      const close=page.getByRole('button',{name:'Close arrival details'});const box=await close.boundingBox();assert.ok(box.width>=44&&box.height>=44);
      session.changedState={at:active.at,text,closeTarget:box};
      ...
      await page.getByText('ETA unavailable',{exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:/Red arrival details/}).count(),0);
      active={...clean,server_eta:null};await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));
      await page.getByText('ETA unavailable',{exact:true}).waitFor();
      active=clean;await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));
      await page.getByRole('button',{name:/Red arrival details: At your stop/}).waitFor();
      assert.equal(await page.getByText('ETA unavailable',{exact:true}).count(),0);
      session.changedState.staleMissingRecovery=true;
...
}finally{await browser.close();await fs.writeFile('/home/gwarren/projects/yale-shuttle-watcher/overnight-2026-09-17/eta/review-round-5'+'/browser-transition.json',JSON.stringify(report,null,2)+'\n');}

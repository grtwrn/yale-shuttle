...
getv=lambda i:dict(db.execute('select * from stop_visits where id=?',(i,)).fetchone());getl=lambda i:dict(db.execute('select * from legs where id=?',(i,)).fetchone());checked=set();sources=set();targets=set();rows=0;legUses=0;totals=collections.Counter();tails=0
for line in (O/'preflight-connected-pairs.jsonl').open():
 ...
 if k not in checked:
  ...
  for lid in r['legIds']:
   l=getl(lid);assert l['route_id']==r['route'] and l['bus_name']==s['bus_name'] and l['departed_at']==cur['departed_at'] and l['from_index']==cur['stop_index'];assert l['from_stop_id']==seq[l['from_index']] and l['to_stop_id']==seq[l['to_index']] and l['reached']==1;assert (l['to_index']-l['from_index'])%len(seq)==l['hops'];vv=list(db.execute("select * from stop_visits where route_id=? and bus_name=? and stop_index=? and anchored_at>=? and anchored_at<=? and how!='gap' and (arrived_at=? or (arrived_at is null and outcome='passed' and departed_at=?))",(r['route'],s['bus_name'],l['to_index'],cur['anchored_at'],l['arrived_at'],l['arrived_at'],l['arrived_at'])));assert len(vv)==1;cur=dict(vv[0]);legUses+=1
...
assert tails==score['counts']['largeBandChanges'];db.close()

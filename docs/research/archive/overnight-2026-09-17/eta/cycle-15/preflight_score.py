...
metrics=collections.defaultdict(lambda: {'current':[],'canonical':[],'source':set(),'target':set()});counts=collections.Counter();availability=collections.Counter();flips=collections.Counter();stability=collections.Counter();previous={};tailFile=(O/'preflight-large-tail-changes.jsonl').open('w');scoredFile=(O/'preflight-connected-pairs.jsonl').open('w');worst=[];progress=collections.Counter();lastTrack={};sampleCases=[]
for aa,bb in zip(gzip.open(O/'current-wire.jsonl.gz','rt'),gzip.open(O/'current-wire.jsonl.gz','rt'),strict=True):
 if counts['polls']>=500:break
 ...
 for key in ka.keys()|kb.keys():
  ...
  if a['sample'] or band>60:
   ...
   if si<0:reason='no-source'
    ...
    # Fixed physical-position occurrence; current source is eligible until departure.
    # Recorded arrival before forecast remains a transition case, never relabeled future.
    eligible=[(i,v) for i,v in enumerate(vs) if v['stop_index']==idx and (i>0 or v['departed_at'] is None or v['departed_at']>=at)]
    if len(eligible)>occ:
     ti,target=eligible[occ];arrival=target['arrived_at'] if target['arrived_at'] is not None else target['departed_at'];legIds=ls[:ti]
...
tailFile.close();scoredFile.close();summary={}

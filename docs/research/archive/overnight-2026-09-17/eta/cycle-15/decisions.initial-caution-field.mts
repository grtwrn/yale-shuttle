...
for(let i=0;i<all.current.length;i++){
 ...
 for(const oa of a.options){const ob=b.options.find((x:any)=>x.routeLabel===oa.routeLabel);if(!ob)continue;if(oa.catchRisk!==ob.catchRisk)caution++;if(!!oa.journeyArrival!==!!ob.journeyArrival)available++;

...
base='1b66a1914a92a79c0936cf65473617cf7b7695f9';head=base
assert git('rev-parse','HEAD').decode().strip()==head
assert git('diff','--cached')==b''
expected_paths={'docs/server-side-eta.md','services/shuttle-v2/web/src/journeyArrival.ts','services/shuttle-v2/web/src/atStopJourney.test.ts','services/shuttle-v2/web/src/TransitMap.tsx'}
assert set(git('diff','--name-only').decode().splitlines())==expected_paths
f='services/shuttle-v2/web/src/TransitMap.tsx';src=(root/f).read_text();before=git('show','HEAD:'+f).decode()
old='journeyArrival(forecastBoard, visits, o.alightStopId, effectiveWalkToSec, o.walkFromSec, nowMs)'
assert before.count(old)==1
assert src==before.replace(old,old[:-1]+", 'at-stop')")

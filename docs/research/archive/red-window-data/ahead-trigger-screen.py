...
plan=dict(createdAtUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),fitEnd=FIT,calibrationEnd=TEST,developmentEnd=END,
  ...
  arrivalProxy='Pinned arrival+15s, including pinned passes, only once a pin occurs. Unpinned visits are not asserted arrivals. Sparse progress uses unconditional anchor+15s for ALL visits. These proxies are not exact event receipt logs.',
  causalCorrection='Corrected after initial screen: never condition anchor emission on whether the visit eventually pins. Original provisional artifacts preserved as before-causal-fix.',
...
planPath=D/'ahead-trigger-plan-v2.json'
if planPath.exists():
...
for v in visits:
 ...
 if v['pinned_at'] is not None and a+15000<=END:events[REGIMES[0]][v['bus_name'],'arrival'].append(dict(arrival,known=a+15000))
 if v['anchored_at']+15000<=END:events[REGIMES[0]][v['bus_name'],'progress'].append(dict(arrival,physical=v['anchored_at'],known=v['anchored_at']+15000))
 if rd is not None and rd<=END:
  if v['pinned_at'] is not None:events[REGIMES[1]][v['bus_name'],'arrival'].append(dict(arrival,known=rd))
  events[REGIMES[1]][v['bus_name'],'progress'].append(dict(arrival,physical=v['anchored_at'],known=rd))
...
def history(r,t,reg):
 ...
 if p:
  es=events[reg].get((p['bus'],'progress'),[]);ts=times[reg].get((p['bus'],'progress'),[]);hi=bisect.bisect_right(ts,t)
  if hi:
   e=es[hi-1]
...
out=dict(method=__doc__,plan=plan,support=support,fits=outputFits,scores=scores,eventStudy=eventStudy,perEpisodeScores=predictions,
 ...
 limitations=['Pinned arrival+15s and clearance+30s are proxies, not exact receipt reconstruction. Unconditional anchor+15s is sparse progress, not asserted stop arrival. Pinned passes count as presence; unpinned passes are progress evidence only.',

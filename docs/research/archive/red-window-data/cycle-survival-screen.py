"""Additional cycle-pulse experiment after initial release integration regressions.
Fixed180sec Gaussian pulse around training-only median own departure cycle.
This is adaptive development; prior results are preserved separately.
...
AGES = [0, 120, 300, 480]
ARMS = ['lap_residual', 'hazard_age_lap_clock15', 'hazard_age_lap_cycle', 'hazard_age_lap_clock15_cycle']
stops = {r['stop']: r for r in source['results']}
CYCLES = {s: statistics.median(r['lap'] + r['y'] for r in rows if r['stop']==s and r['ready']<FIT and r['lap'] is not None and .65*stops[s]['referenceLap']<=r['lap']<=1.65*stops[s]['referenceLap']) for s in [11,121]}
...
def design(r, t, arm):
    ...
    if arm.endswith('_cycle'):
        z = (r['lap'] + t - CYCLES[r['stop']]) / 180 if available else np.zeros_like(t)
        pulse = np.exp(-.5 * z**2) * float(available)
        cols += [pulse, z * pulse]
    ...
    for arm in ARMS:
        if arm.endswith('_residual'):
            ...
            fits[stop, arm] = dict(coefficients=opt.x.tolist(), trainRiskBins=len(y),
                                   trainEvents=int(y.sum()), converged=bool(opt.success), cycleMedian=CYCLES[stop])
            for r in tr + ca + te:
...
(D / 'cycle-survival-screen.json').write_text(json.dumps(out, indent=2) + '\n')
for s in scores:
    if s['calibrated']:

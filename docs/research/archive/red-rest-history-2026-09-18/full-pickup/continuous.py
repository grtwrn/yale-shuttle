...
def analyze(label, folder, dbpath):
    score=json.loads((folder/f'score-{label}.json').read_text())
    ...
    journeys=[j for j in score['journeys'] if j['sourceStop']==11 and j['target']==48 and j.get('sourcePinValid',True)]
    ...
    with (folder/'pairs.jsonl').open() as stream:
        for at,rawframe in itertools.groupby((json.loads(line) for line in stream),key=lambda x:x['at']):
            frame={}
            for raw in rawframe:
                r={**raw,'baseline':raw[label],'candidate':raw['history']}
            ...
            for bus,r in frame.items():
                ...
                if matches:
                    ...
                    if r['occurrence']!=0 or r['stopsAhead']>=route_length:
...
out={'method':__doc__,'data':[analyze(arm,HERE,HERE/'outcomes.db') for arm in ['core','union']]}
(HERE/'continuous.json').write_text(json.dumps(out,indent=2)+'\n')
for d in out['data']:
 print(d['label'])
 for s in d['summary']:
 ...
 for j in d['journeys']:
  ...
  if m and m['value']>0:print('RAISED LOWER AFTER DEPARTURE',j['sourceId'],j['targetVisitId'],j['day'],j['targetOutcome'],json.dumps(m))

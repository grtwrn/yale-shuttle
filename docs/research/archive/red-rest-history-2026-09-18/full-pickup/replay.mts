...
import {offlineHistoryAudit} from './history-release.ts';
import {createHash} from 'node:crypto';
import assert from 'node:assert/strict';
import {computeUpcomingArrivals} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/arrivals.ts';
import {ServerEta} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/src/server/serverEta.ts';
import {registerRoutePaths} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/anchor.ts';
import {applyModelParams} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/eta/params.ts';
import {setReleaseModelEnabled} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/eta/release.ts';
import {setSampledFutureLap} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/eta/arrival.ts';
import {ETA_MAX_AGE_MS} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/etaSource.ts';
import {anchorKeyFor} from '/home/gwarren/projects/yale-shuttle-watcher/route-card-window-2026-09-18/services/shuttle-v2/web/src/liveAnchor.ts';
...
const arm=process.argv[2];assert.ok(['core','union','history'].includes(arm));
...
const frozen=read(O+'../own-history/fits.json'), coreFit={stopId:11,referenceLap:frozen.referenceLap,coefficients:frozen.fits.primary.lap_elapsed_clock.coefficients,n:160,days:6};
...
for(const line of [A+'/release-integration-data/raw-complete-frames.jsonl',O+'raw-today-frames.jsonl'].flatMap(p=>fs.readFileSync(p,'utf8').trim().split('\n'))){
 ...
 for(const b of raw){const belief=store.get(anchorKeyFor('Red',b.bus_name))?.belief;if(belief)fs.writeSync(tracking,JSON.stringify({at,bus:b.bus_name,lead:belief.lead,rested:belief.rested,restSince:belief.restSince,restStop:belief.restStop,lap:belief.lap,pSum:Array.from(belief.p as number[]).reduce((a,b)=>a+b,0),fullBeliefHash:createHash('sha256').update(JSON.stringify(belief,(_k,v)=>ArrayBuffer.isView(v)?Array.from(v as any):v)).digest('hex')})+'\n');}
 for(const a of arrivals){if(![48,4].includes(a.stopId)||a.stopsAhead<1||a.stopsAhead>=60)continue;const name='#'+a.busName,bus=raw.find((b:any)=>b.bus_name===name);assert.ok(bus);const belief=store.get(anchorKeyFor('Red',name))?.belief;assert.ok([a.eta,a.low,a.high].every(Number.isFinite));assert.ok(a.low<=a.eta&&a.eta<=a.high);const row={at,bus:name,busId:bus.bus_id,route:3,target:a.stopId,stopsAhead:a.stopsAhead,occurrence:(a as any).experimentalOccurrence,segmentId,warmMs:at-perBus.get(name).first,observedAt:bus.observed_at??null,source:bus.at_stop_id??null,rested:belief?.rested,since:belief?.restSince,lead:belief?.lead,pinnedSince:bus.at_stop_since??null,lastMovedAt:bus.last_moved_at??null,restStop:belief?.restStop,forecast:{eta:a.eta,low:a.low,high:a.high},departNow:a.departNow,lowFloor:a.lowFloor};fs.writeSync(output,JSON.stringify(row)+'\n');emitted++;}
 last=at;n++;if(n%2000===0)console.error(JSON.stringify({arm,frames:n,emitted}));
}
fs.closeSync(output);fs.closeSync(tracking);const meta={arm,frames:n,emitted,parityChecks,gaps,routes,coreFit,historyAudit:offlineHistoryAudit()};fs.writeFileSync(O+arm+'-meta.json',JSON.stringify(meta,null,2));console.log(JSON.stringify(meta));

...
for r in raw:
    ...
    if r.get('occurrence')!=0:rejected['later target occurrence excluded before first-arrival matching']+=1;continue
...
for s in sourceVisits:
    for target in targets:
        ...
        journeys.append(dict(meta,sourcePinValid=s['id']!=65237,pinnedAt=s['pinned_at'],departedAt=s['departed_at'],targetArrivedAt=arrival,legIds=[l['id'] for l,_ in path],forecastRows=len(candidates)))
        ...
        for phase,e,t in clocks:
            if s['id']==65237 and phase in ['approach','standing']:
                skipped.append(dict(meta,phase=phase,checkpointSec=e,reason='independently verified truncated source pin65237; valid departure retained'));continue

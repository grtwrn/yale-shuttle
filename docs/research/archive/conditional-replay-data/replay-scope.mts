...
const {setConditionalLapScope}=await import(pathToFileURL(root+'/web/src/eta/arrival.ts').href);
...
const arms=['baseline','candidate','current_only','future_only'];
...
for(const f of frames){
 ...
 for(const arm of arms){
  setConditionalLapScope(arm==='current_only'?'current':arm==='future_only'?'future':'both');
...
fs.closeSync(output);const meta={parityChecks,frames:n,emitted,changed,beliefChecks,gaps,params:params.version,priorCutoff:'2026-09-14T04:00:00Z',candidatePatch:process.env.CANDIDATE_PATCH??'normalized-patch.json',note:'Unrounded target-scoped estimator output with periodic full ServerEta parity checks; continuous within observed segments; no synthetic observations or watcher-run resets. Distributions/tables priorSep14; parameter version publishedbeforeSep16. Not original five-second production replay when watcher cadence is ten seconds.'};

...
const out='/home/gwarren/projects/yale-shuttle-watcher/history-recency-data/'+(production?'production-recorded':'recorded');fs.mkdirSync(out,{recursive:true});
...
try{
 ...
 const observed=page.getByRole('region',{name:'Recorded arrival history'});
 const probe=probes.find(p=>p.stop===48).result;
 const {historyRecency}=await import(pathToFileURL(root+'/web/src/historyRecency.ts').href);
 const expected=historyRecency(probe.journey.trips,probe.asOf);
 const circles=await observed.locator('svg circle').evaluateAll(xs=>xs.map(x=>({x:Number(x.getAttribute('cx')),opacity:Number(x.getAttribute('stroke-opacity')),width:Number(x.getAttribute('stroke-width')),title:x.textContent})));
 assert.equal(circles.length,probe.journey.trips.length);
 for(let i=0;i<circles.length;i++){
  assert(Math.abs(circles[i].opacity-(.65+.35*expected.weights[i]))<1e-9);
  assert(Math.abs(circles[i].width-(1+.8*expected.weights[i]))<1e-9);
 }
 assert(new Set(circles.map(x=>x.width)).size>1);
 assert((await observed.innerText()).includes('Recent-weighted median:'));
 assert((await observed.innerText()).includes('weight halves every 2 days'));
 await observed.locator('figure').screenshot({path:out+'/weighted-plot.png'});
 await observed.locator('summary').filter({hasText:'Dates and recorded times'}).click();
 assert.equal(await observed.locator('tbody tr').count(),circles.length);
 await observed.screenshot({path:out+'/weighted-history.png'});
 fs.writeFileSync(out+'/weights.json',JSON.stringify({expected,circles},null,2));

...
import { BLUE_K10_MODELS, applyBlueK10Trial, blueK10GroupPredictions } from './blueK10Trial.js';
...
const fixtures = JSON.parse(gunzipSync(fs.readFileSync(new URL('./__fixtures__/blue-k10-parity.json.gz',import.meta.url))).toString()) as {
...
const models = new Map(BLUE_K10_MODELS.map(m=>[m.routeId,m]));
const routes = Object.fromEntries(BLUE_K10_MODELS.map(m=>[String(m.routeId),m.sequence]));
const fixture = fixtures.find(f=>f.route===16 && f.expected.changed)!;
...
it('matches the independent Python broad-K10 forecast and fallback decisions on both Blue lines', () => {
  expect(fixtures.length).toBeGreaterThan(8000);
  let changed=0;
  for(const f of fixtures) {
    const base=wire(f), trial=applyBlueK10Trial(base,evidence(f),routes);
...
it('pins collector checkpoints to each generated model and never enables Night or Weekend', () => {
  expect([...models.keys()].sort((a,b)=>a-b)).toEqual([1,16]);
  ...
  for(const label of ['Red','Blue Night','Blue Weekend','Green']) {
    const base=wire();base.buses[0]=['42',label,fixture.anchor,null];
    const trial=applyBlueK10Trial(base,evidence(),routes);
    expect(trial.rows).toEqual(base.rows);expect(trial.distributions).toEqual(base.distributions);
  }
});
it('copies the entire live forecast after release and on stale, future, wrong-route or absent evidence', () => {
  ...
  for(const bad of [{...e,released:true},{...e,observedAt:fixture.now-15_001},{...e,observedAt:fixture.now+1},
    ...
    const trial=applyBlueK10Trial(base,new Map([['42',bad]]),routes);
    expect(trial.rows).toEqual(base.rows);expect(trial.distributions).toEqual(base.distributions);
  }
  expect(applyBlueK10Trial(base,new Map(),routes).rows).toEqual(base.rows);
});
...
it('sorts competing arrivals and keeps the matching distribution with each row', () => {
  ...
  expect(trial.trial!.byRoute!['Blue West']).toBe(1);
});

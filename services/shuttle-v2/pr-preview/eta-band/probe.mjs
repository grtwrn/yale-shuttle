// The harness that produced the screenshots in this folder (a copy of
// pr-preview/bunching/probe.mjs with the countdown span found by its FORM
// rather than by the bunched suffix). See README.md.
...
for (const sc of scenarios) {
  ...
  if (MEASURE && sc.measure) {
    const widths = await page.evaluate(({ PILLS, TEXTS }) => {
      ...
      // The countdown span: the one beside the route pill that prints a countdown form.
      const cands = spans.filter((s) => /^(in |now|arriving now|<1 min|\d+ (\(|min))/.test(s.textContent.trim())
        && s.children.length === 0 && s.previousElementSibling && /^(Red|Orange Night|Blue Day)$/.test(s.previousElementSibling.textContent.trim()));
      const eta = cands[0];

/**
 * THE RETIRED ANCHOR — the replay's own copy.
 *
 * `findRouteAnchor` and its dials, verbatim as they last shipped in
 * web/src/anchor.ts (retired 2026-09-06 when the ring estimator took every
 * route). Nothing in the app imports this; `gps-replay.ts` runs it for the
 * `chord` replica (the legacy arithmetic as a counterfactual baseline) and
 * for its anchor-agreement rows, and the historical instruments under
 * scripts/eta-replay/ that measured the anchor still run against it. The
 * measurements in the comments below are the record of why each rule was
 * what it was; read CLAUDE.md's "ETA accuracy" section for what replaced them.
 */
import { distanceToSegmentM, haversineMeters, traceStopLegs } from "../../../web/src/geo";
import type { LatLon } from "../../../web/src/geo";
import { routePathFor } from "../../../web/src/anchor";

/**
 * The stall credit's three bounds, as they last shipped in web/src/arrivals.ts
 * (see gps-replay.ts' `chord` replica): the share of the first hop an
 * elapsed dwell may cancel where no dwell statistic exists, the fastest a
 * shuttle plausibly covers the straight line (calibrator.ts MAX_PLAUSIBLE_M_S,
 * ~79 km/h), and the shortest any hop was billed at.
 */
...
function legGeometry(
  ...
  const path = routePathFor(routeId);

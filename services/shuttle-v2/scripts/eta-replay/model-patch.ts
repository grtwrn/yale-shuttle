 ...
 *
 * WHAT PRODUCTION SERVES IS NOT EVERYTHING THE CALIBRATOR CAN COMPUTE. Every
 * field goes out on `SPLIT_SERVED_ROUTE_IDS` only and never on a `foldRoutes`
 * line; both are read out of the calibrator, so this patch tracks the server
 * when the list changes. `MODEL_ROUTES=all` withholds nothing — deliberately
 * NOT production's behaviour; it exists for the paired before/after rider-sim
 * run CLAUDE.md requires before a route joins the allowlist.
 *
...
const MODEL_ROUTES = process.env.MODEL_ROUTES ?? "served";
if (MODEL_ROUTES !== "served" && MODEL_ROUTES !== "all") {
  throw new Error(`MODEL_ROUTES=${MODEL_ROUTES}: expected "served" (production) or "all"`);
}

/** The file's shape — what `rider-sim/run.ts` reads as `PayloadPatch`. */
interface Patch {
  segments: Record<string, Record<string, Pick<SegmentEntry, "drive" | "driveN" | "dq" | "dqn">>>;
  dwells: Record<string, Record<string, Pick<DwellEntry, "q" | "qn" | "pstop">>>;
...
console.error(
  `calibrated at ${new Date(NOW).toISOString()}: ${stats.segmentCount} segments, ${stats.dwellCount} dwells, ` +
    `${stats.standCount} stand tables, ${stats.driveCount} drives, ${stats.legQuantileCount} hop quantile tables, ` +
...
const withheld = MODEL_ROUTES === "all" ? new Set<number>() : splitWithheldRoutes(net.network);
{
  ...
  console.error(`MODEL_ROUTES=${MODEL_ROUTES}: withheld ${withheld.size} routes, ${standCount} stand tables, ${driveCount} drives, ${dqCount} hop quantile tables, ${paceTable.size} route paces`);
}
...
const rows: Array<{ route: string; hops: number; drives: number; dqs: number; stops: number; stands: number; pace: string }> = [];
for (const r of net.routes) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!s) continue;
    const fields = segmentSplitFields(s);
  ...
  if (Object.keys(segMap).length + Object.keys(dwMap).length > 0 || p) {
    rows.push({
      ...
      stops: new Set(r.stops).size, stands: Object.keys(dwMap).length,

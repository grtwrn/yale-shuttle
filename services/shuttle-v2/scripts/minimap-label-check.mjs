...
try {
    ...
    const first = await capture(`same-route-${desktop ? 1280 : 360}`, true);
    if (baseline) {
        assert(first.buses.every(b => b.name === null));
        assert.equal(first.labels.find(l => l.wait).text, 'Red 3:21/~5m');
        report.checks.push('Reproduced unnamed bus markers and absent visible bus number');
    }
    else {
    ...
    const toggle = page.getByRole('button', { name: 'Fullscreen', exact: true }), target = await toggle.boundingBox();
    ...
    if (baseline) {
        assert.equal((timing.match(/\(B\)/g) || []).length, 4);
        report.checks.push('Reproduced Blue Day/Brown identical (B) tags at both endpoints');
    }
    else {
        assert(timing.includes('(Blue Day) 2–19 min'));
        assert(timing.includes('(Brown) 2–19 min'));
        assert(timing.includes('(R) 2–19 min'));
        assert(!timing.includes('(B)'));
        report.checks.push('Both pickup and arrival time stacks distinguish Blue Day/Brown without color');
    }
    for (const width of [390, 430, 1280, 640]) {
        await page.setViewportSize({ width, height: 844 });
        await page.evaluate(() => window.fixtureMap.invalidateSize());
        await page.waitForTimeout(150);
        await capture('three-routes-' + width);
    }
    await page.getByRole('button', { name: 'Zoom in', exact: true }).focus();
    const zoom = await page.evaluate(() => window.fixtureMap.getZoom());
    await page.keyboard.press('Enter');
    await page.waitForTimeout(150);
    assert.equal(await page.evaluate(() => window.fixtureMap.getZoom()), zoom + 1);
    await page.getByRole('button', { name: 'Zoom out', exact: true }).focus();
    await page.keyboard.press('Enter');
    await page.waitForTimeout(150);
    assert.equal(await page.evaluate(() => window.fixtureMap.getZoom()), zoom);
    report.checks.push('Keyboard zoom in/out reclusters without errors');
    await page.setViewportSize({ width: 360, height: 800 });
    await page.evaluate(() => { const m = window.fixtureMap; m.invalidateSize(); m.panBy([100, -65], { animate: false }); });
    await page.waitForTimeout(200);
    await capture('edge', true);
    await page.evaluate(base => window.setOptions([{ ...base, busWait: null, boardEta: null, arriveAt: null }]), base);
    await page.waitForTimeout(100);
    const missing = await capture('no-forecast');
    assert.equal(missing.labels.length, 0);
    if (!baseline)
        assert.equal(missing.buses[0].name, 'Red #307');
    report.checks.push('Unavailable times remove wait/arrival labels while preserving bus identity');
    await page.locator('.bus-pin-sm').nth(1).focus();
    ...
    await page.evaluate(() => window.setOptions([]));
    await page.waitForTimeout(100);
    const empty = await capture('empty');
    assert.equal(empty.buses.length, 0);
    assert.equal(empty.labels.length, 0);
    report.checks.push('Empty options remove old bus markers and labels');
    await page.evaluate(base => window.setOptions([{ ...base, label: 'Orange Night', passedBus: null, boardEta: '2–119 min', arriveAt: '12:59 PM' }, { ...base, label: 'Orange East', passedBus: null, bus: { ...base.bus, name: '410' }, boardEta: '8–125 min', arriveAt: '1:25 PM' }]), base);
    await page.waitForTimeout(150);
    const long = await capture('orange-long-window', true);
    if (!baseline) {
        const times = long.labels.filter(l => !l.wait).map(l => l.text).join(' ');
        assert(times.includes('(Orange Night) 2–119 min'));
        assert(times.includes('(Orange East) 8–125 min'));
    }
    for (const l of long.labels.filter(l => l.wait))
        assert(l.left >= long.map.left - 1 && l.right <= long.map.right + 1, 'long route wait in bounds');
    report.checks.push('Long route names and broad pickup windows remain present without clipping numerical ranges');
    for (const state of report.states) {
        assert.equal(state.overflow, false, state.name + ' page reflow');
        for (const label of state.labels.filter(l => l.wait)) {
            ...
            for (const other of others)
                assert(label.right <= other.left + 1 || label.left >= other.right - 1 || label.bottom <= other.top + 1 || label.top >= other.bottom - 1, state.name + ' wait avoids another label or bus');
        }
    }

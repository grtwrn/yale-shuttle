...
// centre near 0. Measured over the 3-9 Sep archive, 9,342 observed berths in 235
// cells: median per-cell offset +7.2 m, median per-cell 10-90 span 64.4 m - so
// the published coordinates are, in the main, correct.
//
// (That pair read 3.6 m / 41.6 m before the last-stand rule below, and the
// header comment here said 38.8 m / p90 +8.6 m. Both were measured by the FIRST
// version of this script and neither survives it: same six days, last-fix
// 8,778 berths / 3.6 m / 41.6 m / 4 qualifying, last-stand 8,300 / 6.8 / 64.9 /
// 9. The span widens because a visit where nobody boards leaves its last stand
// at the queue behind the kerb - which is exactly why the decision scores the
// window against the visits NOT behind it rather than against the spread.)
//

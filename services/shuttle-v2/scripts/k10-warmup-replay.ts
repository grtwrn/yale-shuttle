...
for (const f of features) {
  while (cursor < raw.length && raw[cursor].collected_at <= f.asof) {
    ...
    if (day !== group[0].day) { states.clear(); visits.clear(); clock = new K10Clock(); tracker = new K10Tracker(); day = group[0].day; }
  ...
  if (compared % 35 === 0) {
    ...
    if (current) {
      ...
      restarted.update(net, [current], o => raw.filter(r => r.bus_name === o.busName
        && r.collected_at >= o.collectedAt-3_600_000 && r.collected_at < o.collectedAt)
        .map(r => ({ busId:r.bus_id,busName:r.bus_name,routeId:r.route_id,lat:r.lat,lon:r.lon,
          heading:r.heading,lastStopId:r.last_stop_id??null,collectedAt:r.collected_at })));
      const got = restarted.snapshot(f.asof).get(f.bus.replace(/^#/,''));
      assert.deepEqual(got, e, `restart ${f.bus} ${f.asof}`);
      restarts++; if (got) recovered++;
      maxReplayMs = Math.max(maxReplayMs, restarted.stats().maxReplayMs);
    }
  }
...
assert(restarts > 100); assert.equal(recovered, restarts);
assert(compared > 3000); assert(active > 1000); assert(released > 100);
console.log(JSON.stringify({compared, active, released, restarts, recovered, maxReplayMs, assertion:'live clock agrees with frozen causal reference'},null,2));

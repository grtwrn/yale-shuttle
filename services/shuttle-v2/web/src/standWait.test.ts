...
import { standingForecastsFor, type StandingForecastPrior } from "./eta/standingForecast";
...
describe("standWaitFor — composed from what a render site already holds", () => {
  ...
  it("uses the visit forecast for the shared pause interval, countdown range and running-long cue", () => {
    const arrival = Date.parse("2026-09-08T12:00:00Z");
    const duration = fromQuantiles(WINCHESTER_Q);
    const context: StandingForecastPrior = {
      route_id: 3, route_pattern_id: "pause-ui-context", canonical_stop_ids: [11, 27],
      stop_id: 11, stop_index: 0, observed_visit_start_at: arrival,
      previous_departed_at: arrival - 3_600_000, history_available_at: arrival - 3_500_000,
      phase_slot_at: arrival + 900_000,
      phase_error_q: [-120, -90, -60, -30, 0, 30, 60, 90, 120, 180], phase_weight: 1,
      duration_dist: { xs: [...duration.xs], ps: [...duration.ps], tail_hazard: duration.tailHazard },
      fitted_at: arrival - 86_400_000, valid_until: arrival + 86_400_000,
    };
    const now = arrival + 540_000;
    const forecasts = standingForecastsFor({ route_id: 3, standing_forecasts: [context] }, { routeId: "3", stops: [11, 27], N: 2 }, now);
    const standing = { stopId: 11, standingSec: 540 };
    const baseline = standWaitFor(standing, ROUTE, undefined, 600, 27)!;
    const contextual = standWaitFor(standing, ROUTE, undefined, 600, 27, { forecasts, stopId: 11, stopIndex: 0, now })!;
    expect(contextual.remainingSec).toBeGreaterThan(baseline.remainingSec + 150);
    expect(contextual.soonSec).toBeGreaterThan(200);
    expect(contextual.lateSec).toBeGreaterThan(400);
    expect(contextual.range).not.toBeNull();
    expect(contextual.range).not.toEqual(baseline.range);
    expect(contextual.leftText).not.toBe(baseline.leftText);
    expect(baseline.overdue).toBe(true);
    expect(contextual.overdue).toBe(false);
  });

  it("gates display on the standing quantiles' sample count when available", () => {
    const standing = { stopId: 11, standingSec: 60 };
    expect(standWaitFor(standing, { "11": { ...WINCHESTER, n: 0, qn: 3 } }, undefined, 400, 27)).not.toBeNull();
    expect(standWaitFor(standing, { "11": { ...WINCHESTER, n: 100, qn: 2 } }, undefined, 400, 27)).toBeNull();
    expect(standWaitFor(standing, { "11": { med: 300, sd: 30, n: 3, q: WINCHESTER_Q } }, undefined, 400, 27)).not.toBeNull();
  });
});

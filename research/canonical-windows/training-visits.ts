...
const output = 'research/canonical-windows/results/';
...
// Preserve the raw sequence for an audit; every computational consumer uses
// the runtime network sequence serialized below, including repeated stops.
...
const canonical = { stops: topology.stops, routes: [...network.routes.values()] };
const rebuilt = TransitNetwork.build(canonical.stops, canonical.routes);
assert.deepEqual([...rebuilt.routes].map(([id,r])=>[id,r.stops]), [...network.routes].map(([id,r])=>[id,r.stops]), 'Canonical rebuild changed occurrence indices');
const canonicalBytes = JSON.stringify(canonical);
fs.writeFileSync(output + 'canonical-topology.json', canonicalBytes + '\n');
const canonicalSha256 = createHash('sha256').update(canonicalBytes + '\n').digest('hex');
...
const audit = { input, inputSha256: rawHash.digest('hex'), inputRows,
  ...
  canonicalSha256,
  topologyPolicy: 'Runtime canonical sequence is authoritative. Original indices and repeated occurrences are preserved; only invalid runtime occurrences are excluded.',

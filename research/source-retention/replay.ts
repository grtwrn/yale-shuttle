...
import {Observer,KS} from '../source-discard/observer.ts';
import {capFor as sourceCap,retain,observeRelease} from './rules.ts';
export function replay(net:any, top:any, waits:any, raw:any[], preds:any[], capMinutes=45, delay=0){
 const capFor=(route:number)=>sourceCap(route,capMinutes);
 ...
 for(const p of preds){
  ...
  while(cursor<raw.length&&raw[cursor].collected_at<=asof){
   ...
   for(const [name,s] of states){
    ...
    for(const w of waits[s.routeId]??[])for(const k of ks){
     ...
     observeRelease(latches,`${k}/${w}`,origin,release,index,w,k,n,phase,began,time,capFor(s.routeId));
    }
   }
  }
  ...
  const origins:any={};
  if(ready)for(const[i,e]of histories.get(p.bus_name)??[])if(retain(e,p.route_id,asof,began,p.predicted_at,capFor(p.route_id)))origins[i]=e;
  if(!ready)unknown++;
  ...
  // Provenance observation still describes original45 filtering; treatment never
  // reads the diagnostic ledger to recover a missing model history entry.
  const originalOrigins=Object.fromEntries(Object.entries(origins).filter(([,e]:any)=>row.at-e.departed<=2700000));
  const diagnostic=observer.snapshot({...row,origins:originalOrigins},s,v,w,histories.get(p.bus_name));
  const proofs:any={};
  for(const [i,e]of Object.entries(origins) as any){
   const matches=observer.matches(row.bus,Number(i),e);
   if(matches.length!==1)throw Error('unproven/ambiguous retained physical source');
   const evidence=matches[0];
   if(!(e.departed<=e.knownAt&&e.knownAt<=asof))throw Error('noncausal retained source');
   proofs[i]={emission:evidence.id,provider:evidence.provider,anchorProvider:evidence.anchorProvider,
    departedAt:evidence.departedAt,knownAt:evidence.knownAt,ageSec:(row.at-e.departed)/1000};
  }
  diagnostics.push({...diagnostic,retentionCapMinutes:capFor(row.route)/60000,retainedPhysicalProofs:proofs});
  rows.push(row);
 }

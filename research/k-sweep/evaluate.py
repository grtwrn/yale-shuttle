"""Occurrence-aware all-route transfer test. Freeze forecasts before attaching test outcomes."""
...
class Models:
    def __init__(self,visits,quality):
        ...
        for (bus,rid),vs in groups.items():
            ...
            for w in WAITS[rid]:
                for k in KS:
                    if k<n:sources[(w-k)%n].append((k,w))
    ...
    def predict(self,r,arm):
        ...
        if n<=k:return dict(base,reason='K exceeds single-occurrence loop support')
        ...
        if r.get('releasedOrigins',{}).get(f'{k}/{w}')==origin['departed'] or (release and release['departed']>origin['departed']) or distance((w-k)%n,index,n)>k or (index==w and r['phase']=='drive'):

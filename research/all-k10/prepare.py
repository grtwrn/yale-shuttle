...
DATA=HERE.parent/'blue-k10'/'data'
...
def main():
    ...
    for s in manifest['sources']:assert hashlib.sha256((DATA/s['path']).read_bytes()).hexdigest()==s['sha256']
    tables={}; counts={}
    for table in ('raw_positions','stop_visits','legs','predictions_log'):
        rows=[]
        for file in sorted(DATA.glob('*/'+table+'.jsonl.gz')):rows+=read(file)

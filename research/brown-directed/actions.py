"""Matched action risks, including all-four common status support."""
import collections
import json
...
def main():
    ...
    for r in old:
        arm = r['arm'] if r['arm'] == 'deployed' else r['arm']+'_original'
    ...
    for (policy,walk,response), rows in sorted(groups.items()):
        ...
        for arm in ARMS:
            summaries.append(dict(arm=arm,policy=policy,walkSec=walk,responseSec=response,
                ...
                allFourCommon=score_pairs([(d,indexed[arm,key(d)]) for d in common])))
        for lead in LEADS:
            for a,b in (('original','directed'),):
                aa,bb=lead+'_'+a,lead+'_'+b
                contrasts.append(dict(reference=aa,candidate=bb,policy=policy,walkSec=walk,responseSec=response,
                    sameAllFourCommon=score_pairs([(indexed[aa,key(d)],indexed[bb,key(d)]) for d in common])))
    result=dict(originalDeployedAndLeadActionIdentity=len(old),pointDecisionChecks=point_checks,
        ...
        note='Same physical visits and deployed arming. Small common support and censoring; no actual rider-miss claim or app recommendation invariance claim.')

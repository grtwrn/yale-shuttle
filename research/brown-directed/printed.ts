...
const base = 'research/brown-directed/', input = base+'results/', out = input;
const arms = ['frozen_K8','rolling_K5'].flatMap(m => ['original','directed'].map(c => `${m}_${c}`));
...
for await (const line of readline.createInterface({input:fs.createReadStream(file).pipe(zlib.createGunzip())})) {
  ...
  unionRows++;
  const scopes = ['fourArmUnion',r.deployedChanged ? 'deployedCheckpointApplied' : 'deployedLiveFallback'];
  if (['frozen_K8_original','rolling_K5_original'].some(a=>differs(r.candidates[a],r.deployed))) scopes.push('originalLeadUnion');
...
const report=['# Printed-window diagnostic','','Fixed directed-leg hybrid forecasts on reused dates. Same four-arm route union; equal weight per physical visit. No model selection or rollout.','',
  '| Line | Arm | Visits | Raw width, s | Printed width, s | Printed narrowing, s | Same text | Printed coverage |',
  '|---|---|---:|---:|---:|---:|---:|---:|'];
for(const r of results.filter(r=>r.scope==='fourArmUnion'&&r.ageSec===0)) {

"""Hosted factorial; causal refit at fixed cutoffs, then immutable label join."""
import collections
import gzip
import hashlib
...
CELLS = ('h45_f15', 'h45_f45', 'h90_f15', 'h90_f45')
...
def main():
    ...
    features = {key(r): r['variants'] for r in read(OUT / 'clock-features.jsonl.gz')}
    ...
    original_union = [r for r in scored if any(r['candidates'][a + '_h45_f15'] != r['deployed'] for a in LEADS)]
    scopes = {'all': scored, 'eightArmUnion': union, 'originalLeadUnion': original_union}
    summary = dict(arms=ARMS, leads=LEADS, generated=len(generated), labelled=len(scored),
        ...
        note='Fixed eight-arm development comparison. Exact deployed point/early protection; no promotion or new dates.')
    ...
    audit = dict(rawHashes=raw_hashes, priorWindowHashes=before, priorWindowBytesUnchanged=True,
        checks=dict(checks), generatedRows=len(generated), labelledRows=len(scored),
        originalFeatureAndLeadIdentity=True, unscoredPersistedBeforeLabels=True,

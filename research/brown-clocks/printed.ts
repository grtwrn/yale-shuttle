...
const base = 'research/brown-clocks/', input = base+'results/', out = input;
const arms = ['frozen_K8','rolling_K5'].flatMap(m => ['h45_f15','h45_f45','h90_f15','h90_f45'].map(c => `${m}_${c}`));
...
for await (const line of readline.createInterface({input:fs.createReadStream(file).pipe(zlib.createGunzip())})) {
  ...
  unionRows++;
  const scopes = ['eightArmUnion',r.deployedChanged ? 'deployedCheckpointApplied' : 'deployedLiveFallback'];
  if (['frozen_K8_h45_f15','rolling_K5_h45_f15'].some(a=>differs(r.candidates[a],r.deployed))) scopes.push('originalLeadUnion');
...
// Formatter copied from the independently checked audit81d7b2a; same production helper.
// Only paired cohort/arm enumeration and output names change.
const sourceAudit=JSON.parse(fs.readFileSync(input+'audit.json','utf8'));
assert(sourceAudit.generatedRows>0);
assert.equal(rows,sourceAudit.labelledRows);assert.equal(labelled,sourceAudit.labelledRows);assert.equal(hash(file),before);
...
fs.writeFileSync(out+'printed-summary.json',JSON.stringify({referenceWindowRun:35685527686,forecastSha256:before,
...
const report=['# Printed-window diagnostic','','Fixed clock-factor hybrid forecasts on reused dates. Same eight-arm route union; equal weight per physical visit. No model selection or rollout.','',
  '| Line | Arm | Visits | Raw width, s | Printed width, s | Printed narrowing, s | Same text | Printed coverage |',
  '|---|---|---:|---:|---:|---:|---:|---:|'];
for(const r of results.filter(r=>r.scope==='eightArmUnion'&&r.ageSec===0)) {
...
fs.writeFileSync(out+'PRINTED.md',report.join('\n'));

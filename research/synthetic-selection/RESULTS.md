...
The input API expects **already-verified** public receipt envelopes. A capture-journal/body-hash decoder, causal release-metadata join, fixed 42-scenario enumerator and bounded streaming runner for all 28,224 profiles still need separate implementation/review before prospective execution. The current envelope fixture buffers diagnostic rows and is not a whole-week scale test. Timer/receipt ties and the 15-second observer cadence remain explicit limitations. Unfinished capture horizons are reported independently of simulated clock completion.

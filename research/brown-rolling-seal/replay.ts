...
const out='research/brown-rolling-seal/replay-results/';
const frozen='research/brown-rolling-seal/input/canonical/canonical-windows/results/';
...
const raw=(await read('research/brown-rolling-seal/input/merged-raw.jsonl.gz')).map((r):BusObservation=>({
...
assert(raw.at(-1)!.collectedAt<Date.parse('2026-09-22T04:00:00Z'),'Unexpected post-cutoff input');
...
assertRows(baseline.rows.filter(r=>r.known_at<Date.parse('2026-09-21T04:00:00Z')),frozenRows,'Original historical prefix differs from frozen canonical visits',signature);
...
const strictSource=(r:any)=>r.pinned_at!==null&&Number.isFinite(r.arrived_at)&&Number.isFinite(r.departed_at)
 &&r.how!=='gap'&&['stopped','passed'].includes(r.outcome)&&r.departed_at<=r.known_at;
const sourceLedger=(rows:any[])=>rows.filter(r=>r.route_id===19&&strictSource(r)).map(identity).sort();
assertRows(sourceLedger(baseline.rows),sourceLedger(candidate.rows),'Strict Brown physical sources');
const prefixes=[];
for(const protectedArm of [false,true])for(const cutoff of [Date.parse('2026-09-16T04:00:00Z'),
 Date.parse('2026-09-21T04:00:00Z'),Date.parse('2026-09-21T16:00:00Z')]){
 const full=protectedArm?candidate:baseline;
 const prefix=replay(raw.filter(o=>o.collectedAt<cutoff),protectedArm,`prefix-${protectedArm}-${cutoff}`,false);
 assertRows(prefix.rows,full.rows.filter(r=>r.known_at<cutoff),'Future-prefix visits');
 const eventCheck=await compareEvents(prefix.events,full.events,false,cutoff);
 prefixes.push({protectedArm,cutoff,visits:prefix.rows.length,events:eventCheck.count,eventSha256:eventCheck.sha256});
 await compressEvents(prefix.events);
 progress('prefix-identity-passed',{protectedArm,cutoff,...eventCheck});
}
...
const result={guardSha256:hash('research/brown-directed/guard.ts'),baselineCanonicalIdentity:true,
 ...
 modelsFitted:0,scoresProduced:false,labelsChanged:false,strictPhysicalSourceParity:true,sourceLedgerCount:sourceLedger(baseline.rows).length,
 rawSha256:hash('research/brown-rolling-seal/input/merged-raw.jsonl.gz'),canonicalVisitsSha256:hash(frozen+'training-visits.jsonl.gz')};
